...
- **Agent tool fidelity:** `STABLE_BIN=target/debug/engram tools/test-harness/bin/engram-harness.sh --suite agent-tool-fidelity --workspace /path/to/Engram --record-results` — asserts `suite_result.passed`, `fidelity_rate >= 0.95`, composite tool registration, `prev_in_trace` lineage, misuse scar + `failure_pattern`, and `post_edit_palette` safe path. Rebuild + restart MCP after substrate changes (`scripts/install-engram-plugin.sh`).
