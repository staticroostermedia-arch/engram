...
**Evidence:** session scratch excerpts (`theory-canon-excerpts/`, `engram-current-excerpts/`)
...
| 6 | **Shock / ΔV recovery** (event-triggered lawful reset) | **partial** | gTVD: *"Event-triggered jump map when a guard trips"* Personal Seed: *"Drift is detected when"* lawful shocks. | `ego_snapshot.drift_velocity` surfaced at wake; `scar` for dead ends. No automated shock→rehydrate playbook when drift exceeds theory threshold; no drift guard in harness injection. |
...
| **Required A/D/R at forks (triadic trace v2)** | Golden Legominism: A/D/R *at least once per section*; gTVD treats triad as state. Optional fields → agents skip them. | `mcp.rs:1085–1140` `inputSchema` — change `affirm`/`deny`/`reconcile` to required when `ENGRAM_PROFILE=agent` OR add `processes/ritual/triadic_trace.toml` gate. Handler already serializes fields (`mcp.rs:4897–4741`). `edit_fidelity.rs` trace-mint path — parallel helper lacks A/D/R (decision+why only). |
...
| **Ritual template** | Add `processes/monitor/sentinel.toml`; extend `turn_record` to increment `var:turns_since_handoff`; fork-trace at rehydrate with decision *"Sentinel threshold hit"*. Pre: `get_backend_readiness`; post: `session_end` + `session_start`. |
...
| **Ritual template** | Update `AGENTS.md` + `engram-working-memory.md`; soft gate in `mcp.rs` fork-trace handler (warn) → hard gate via `ENGRAM_TRIADIC_TRACE=1`. Align `edit_fidelity.rs` trace-mint path or deprecate for agent path. |
...
| TVD at forks | fork-trace optional A/D/R | Weak enforcement |
