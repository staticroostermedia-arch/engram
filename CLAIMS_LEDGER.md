...
| Skill auto-distillation drafts + promote/scar | COGNITIVE_OS_EXTENSIONS E2 | implemented | `skill_distill.rs`; MCP promote persists `receipt:skill_promote_*` / `scar:skill_draft_failed_*` | `skill_distill::tests::*`; promote path stores blocks | Auto-pin only with `ENGRAM_DISTILL_AUTO_PIN=1` |
| Counterfactual branch memory | COGNITIVE_OS_EXTENSIONS E3 | implemented | `branch_memory.rs`; `tag_write` on every successful `StoreHandle::store` (traces/remember/update); anchors filter | `store_path_branch_tags_traces`; `branch_quick_trace_omitted_from_mainline_anchors` (handle_tool_call) | Process-local tags v1 (not multi-host CRDT) |
...
| Multi-agent single-key leases + conflicts | COGNITIVE_OS_EXTENSIONS E5 | implemented | `lease_conflict.rs`; remember/update gated when `ENGRAM_LEASE_ENFORCE=1` or agent profile | `lease_conflict::tests::*`; `lease_blocks_remember_when_enforced` | Single-key TTL; conflict blocks minted on refuse |
...
| Foreign external knowledge stratum | COGNITIVE_OS_EXTENSIONS E9 | implemented | `foreign_stratum.rs`; `recall_scoped(anchors)` omits foreign until accept | `foreign_stratum::tests::*`; `foreign_omitted_from_anchors_until_accept` | URL fetch gated; `scope=all` still sees foreign |
...
| LoRA real weight train | scripts/lora_loop | partial dry-run; train blocked scar | dry-run receipt; no promote without metrics | lora dry-run + blocked receipt | D1: no fake adapter |
