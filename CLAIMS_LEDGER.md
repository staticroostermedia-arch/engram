...
| Footer = cryptographic hash of header+body | PATENT-NOTICE.md L25 | partial | intended: `engram_core` seal into `sig_5`; today: `sig_0` is text/q hash, not full header+body | seal unit tests (when present under `feat/block-integrity-sig5`) | Patent table describes the format; reference encoder historically under-filled `sig_5` |
...
| Homomorphic + ZK transforms | CHANGELOG; encode.rs L370–403 | partial | `apply_homo_op`, `generate_zk_proof`, `verify_zk_proof` | encode tests | **Not** zero-knowledge SNARK/STARK. “ZK” = BLAKE3 attestation of allowed-transform path under CRS |
...
| Lawfulness: `verify_manifold_integrity` / `verify_block_lawfulness` | AGENT_INTEGRATION_GUIDE; LAWFULNESS doc | implemented (tools) / partial (deep chain audit) | `store.rs` `verify_manifold_integrity`; mcp handlers | store manifold integrity tests | Spec in `docs/LAWFULNESS_VERIFICATION_PRIMITIVES.md` is still partly design; runtime is sampling + soft contracts |
...
| Trust residual / mutual morning packet | PR #210; wake path | implemented (on branch) | `store.rs` `build_trust_residual`; `wake_bundle`; mcp wake hoist | `trust_residual_v1_bootstrap_and_handoff`; wake_bundle tests | Merges when PR #210 greens |
