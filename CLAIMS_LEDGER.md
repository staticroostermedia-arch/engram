...
| Protocol invoke MCP | mcp tool list + store | partial (experimental) | `invoke_protocol` → 7-point gate + TOML load + tool bind + receipt | `protocol_invoke_runs_real_toml_and_emits_receipt` | Tools bound/declared, not live MCP graph execution. Prefer `dry_run=true` |
...
| Cold-start fidelity median ≥0.90 (10 live wakes) | goal cognitive completion | implemented | `cold_start_fidelity` on session_start; live stalk ~101k | SCRATCH `csf-median-proof.txt` median=0.951923; series 10 wakes | Wake1 can be low without primary_goal; wakes 2–10 ~0.95 with primary restored |
