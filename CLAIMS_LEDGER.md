...
| Counterfactual branch memory | COGNITIVE_OS_EXTENSIONS E3 | implemented | `branch_memory.rs`; `tag_write` on every successful `StoreHandle::store`; anchors filter | `cognitive_os_acceptance::e3_branch_store_and_quick_trace_isolated` | Process-local tags v1 (not multi-host CRDT) |
