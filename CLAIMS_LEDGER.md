...
| Counterfactual branch memory | COGNITIVE_OS_EXTENSIONS E3 | implemented | `branch_memory.rs`; `tag_write` on remember/update; anchors filter via `concept_visible_in_anchors` | `branch_memory::tests::*`; `branch_tag_write_and_anchors_isolation` | Process-local tags v1 (not multi-host CRDT) |
