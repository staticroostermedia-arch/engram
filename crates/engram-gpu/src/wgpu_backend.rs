...
impl WgpuBackend {
    ...
    pub fn new(store_path: impl AsRef<Path>) -> Result<Self> {
        ...
        std::fs::create_dir_all(&store_path).context("Failed to create manifold directory")?;

        let cpu = CpuBackend::new(&store_path);

        // ── Async GPU initialisation (driven synchronously via pollster) ──
        let (raw_device, queue) =
            pollster::block_on(Self::init_gpu()).context("WebGPU initialisation failed")?;
    ...
    async fn init_gpu() -> Result<(wgpu::Device, wgpu::Queue)> {
        ...
        device.on_uncaptured_error(Box::new(|err| {
            eprintln!(
                "[engram-gpu/wgpu] Uncaptured device error (lost?): {:?}. Consider reinitialize.",
                err
            );
    ...
    fn dispatch_chunk(&self, query_packed: &[u32; 96], chunk: &[PackedBlock]) -> Vec<f32> {
        let n = chunk.len();
        if n == 0 {
            return Vec::new();
        }
...
impl VsaBackend for WgpuBackend {
    ...
    fn store(&self, concept: &str, block: Leg3Pointer) -> Result<()> {
        ...
        let provlog =
            String::from_utf8_lossy(block.payload.split(|&b| b == 0).next().unwrap_or_default())
                .into_owned();
        let (_, packed_arr, _) = quantize_centroid_int8(&block.q);
